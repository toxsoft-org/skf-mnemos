...
public class RtcImage
   ...
   */
  public static final String FACTORY_ID = MNED_LITE + ".rtc.Image"; //$NON-NLS-1$
  ...
  public static final IRtControlFactory FACTORY = new AbstractRtControlFactory( FACTORY_ID, //
    ...
    protected ITinTypeInfo doCreateTypeInfo( IStridablesListEdit<ITinFieldInfo> aFields ) {
      aFields.add( TFI_IMAGE_DESCRIPTOR );
      aFields.add( TFI_X );
      aFields.add( TFI_Y );
      aFields.add( TFI_WIDTH );
      aFields.add( TFI_HEIGHT );
      return new PropertableEntitiesTinTypeInfo<>( aFields, RtcImage.class );
    }

    @Override
    protected void bindViselProps() {
      bindViselPropId( TFI_IMAGE_DESCRIPTOR.id(), TFI_IMAGE_DESCRIPTOR.id() );
    ...
    protected IRtControl doCreate( IRtControlCfg aCfg, VedScreen aVedScreen ) {
      ...
      if( v != null ) {
        ...
        return new RtcImage( cfg, propDefs(), aVedScreen );
      }
      return null;
    }
  };

  protected RtcImage( IRtControlCfg aConfig, IStridablesList<IDataDef> aPropDefs, VedScreen aVedScreen ) {
