...
public class RtcAttrCheckbox
  ...
  public static final IRtControlFactory FACTORY = new AbstractRtControlFactory( FACTORY_ID, //
    ...
    protected ITinTypeInfo doCreateTypeInfo( IStridablesListEdit<ITinFieldInfo> aFields ) {
      ...
      return new PropertableEntitiesTinTypeInfo<>( aFields, RtcAttrCheckbox.class );
    }
    ...
    protected IRtControl doCreate( IRtControlCfg aCfg, VedScreen aVedScreen ) {
      ...
      if( aCfg.viselId().isBlank() ) { // создание с нуля
        ...
        VedItemCfg viselCfg = createViselCfg( ViselCheckbox.FACTORY_ID, aVedScreen, "AttrCheckbox" ); //$NON-NLS-1$
        ...
        IVedActorFactory af = actorFactory( LiteActorAttrCheckbox.FACTORY_ID, aVedScreen );
      ...
      if( v != null && actor != null ) {
        ...
        return new RtcAttrCheckbox( cfg, propDefs(), aVedScreen );
      }
      return null;
    }
  ...
  protected RtcAttrCheckbox( IRtControlCfg aConfig, IStridablesList<IDataDef> aPropDefs, VedScreen aVedScreen ) {
    ...
    if( aConfig.params().hasKey( IRtControlCfg.PROPID_ACTORS_IDS ) ) {
      ...
      if( actor.props().hasKey( TFI_GWID.id() ) ) {
        ...
        if( !gwid.isMulti() ) {
          Ugwi ugwi = UgwiKindSkAttr.makeUgwi( gwid.skid(), gwid.propId() );
          props().setValobj( TFI_ATTR_UGWI.id(), ugwi );
        }
      }
    }
  }
  ...
  protected void updateActorProps( IOptionSet aChangedValues ) {
    if( actor != null ) {
      if( aChangedValues.keys().hasElem( TFI_ATTR_UGWI.id() ) ) {
        Ugwi ugwi = aChangedValues.getValobj( TFI_ATTR_UGWI.id() );
