...
/**
 * Разрешает UGWI типа {@link UgwiKindSkLinkInfo} в UGWI типа {@link UgwiKindSkSkid}.
 * <p>
 * То есть из абстрактного UGWI делает конкретный, получая на вход UGWI типа {@link UgwiKindSkSkid}, а затем из
 * множества объектов, на которые ссылается link, выбирает один.
 *
 * @author vs
 */
public class LinkInfoResolver
    extends AbstractSimpleResolver {

  /**
   * ИД фабрики
   */
  public static final String FACTORY_ID = "linkInfoResolverFactory"; //$NON-NLS-1$
  ...
  public static final ISimpleResolverFactory FACTORY = new AbstractSimpleResolverFactory( //
    ...
    protected AbstractSimpleResolver doCreateResolver( IOptionSet aResolverConfig, ISkConnection aSkConn ) {
      return new LinkInfoResolver( aResolverConfig, aSkConn );
    }
  };
   ...
   */
  public LinkInfoResolver( IOptionSet aResolverConfig, ISkConnection aSkConn ) {
  ...
  protected Ugwi doResolve( Ugwi aMaster ) {
    ...
    if( aMaster.kindId().equals( UgwiKindSkSkid.KIND_ID ) ) {
      ...
      Ugwi linkUgwi = UgwiKindSkLinkInfo.makeUgwi( masterSkid, UgwiKindSkLinkInfo.getLinkId( ugwi ) );
      ISkidList skids = ugwiService.findContentAs( linkUgwi, ISkidList.class );
